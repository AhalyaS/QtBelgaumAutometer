/****************************************************************************
** Meta object code from reading C++ file 'doodle.h'
**
** Created: Mon 31. Oct 13:08:09 2011
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../doodle.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'doodle.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Doodle[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       8,    7,    7,    7, 0x0a,
      23,    7,    7,    7, 0x0a,
      38,    7,    7,    7, 0x0a,
      53,    7,    7,    7, 0x0a,
      64,    7,    7,    7, 0x0a,
      79,    7,    7,    7, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_Doodle[] = {
    "Doodle\0\0createSmoke1()\0createSmoke2()\0"
    "createSmoke3()\0delTimer()\0endAnimation()\0"
    "switchScreen()\0"
};

const QMetaObject Doodle::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_Doodle,
      qt_meta_data_Doodle, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Doodle::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Doodle::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Doodle::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Doodle))
        return static_cast<void*>(const_cast< Doodle*>(this));
    return QDialog::qt_metacast(_clname);
}

int Doodle::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: createSmoke1(); break;
        case 1: createSmoke2(); break;
        case 2: createSmoke3(); break;
        case 3: delTimer(); break;
        case 4: endAnimation(); break;
        case 5: switchScreen(); break;
        default: ;
        }
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
